...
В первой части исследования было произведено сравнение распределения семи хеш-функций.
...
#### Хеш-функции:
1. **hashZero** - всегда возвращает ноль.
...
Из графика видно, что в этой функции присутствуют отчетливые максимумы. Если размер массива будет не простым числом, то они могут накладываться друг на друга, что вызовет сильное ухудшение распределения этой функции.

Сравним распределение hashStrSum при размере массива 1013 и 950 
![](https://github.com/FireKOT/Hash-Table/blob/main/plots/hashStrSumBadSize.png)

Можно заметить, что при размере 950 супремумы накладываются, из-за чего значительно увеличиваются размеры конкретных списков.

Этот пример показывает, что использование в качестве хеш-функции периодическую функцию нежелательно, особенно, если размер массива не является простым числом.
...
Поскольку в базовой версии программы измерения проходят в условиях, аналагичных условиям измерения графиков, то можно заметить, что в данной версии высокая заселенность, равная 57, поэтому первой оптимизацией был увеличен размер таблицы до 11777, чтобы снизить заселенность до 5.
...
Поэтому была написана на ассемблера функция hashTableFindAsm, в которой было оптимизировано обращение к памяти и заинлайнено сравнение строк.
...
На основе произведенного исследований можно заключить, что:
* Не всякую функцию можно брать в качестве хеш-функции из-за возможной плохой равномерности.
* Компилятор GCC с флагами -O1 и -mavx2 компилирует не самую оптимизированную хеш-таблицу.
* При реализации хеш-таблицы крайне важно подобрать подходящий размер массива. 
* Правильным выбором размера и ассемблерными оптимизациями можно добиться ускорения в 4.1 раза, только ассемблерными оптимизациями в 1.7 раз.

Таким образом, если нет конкретной задачи оптимизации с помощью ассемблера, то достаточно ограничиться оптимизацией без него.
